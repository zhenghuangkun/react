/****添加课程管理*******/
var coursePicFlag = false;
var experimentModifyFlag = false;  // true 修改
var experimentGuideModifyFlag = false; // true 修改实验指南， false 没有修改
...
$(function () {
    ...
    // 初始化共享范围是学院时提示文本
    var collegeName = $('#collegeId option:selected').text();
    
    $("#collegeOpenRangeLabel").text(collegeName);
...
function courseExperimentTableInit(){
	courseExperimentTB = $('#courseExperimentTB').DataTable({
		ajax: {
			url: contextPath + '/course/getCourseExperiment',// 数据请求地址
			type: "POST",
			data: function (params) {
				//此处为定义查询条件 传给控制器的参数
				params.courseId = $('#courseUuid').val();
			}
		},
		stateSave:false,
	    lengthChange: true,
		...
		columns: [
		       /*{
				...
				"render": function (data, type, row, meta) {
				   ...
				   "width": "18%",
				   "render":function(data, type, row, meta){
					   return row.experimentDescription;
				   }
			   },{
...
var deleteExperimentArray = new Array();
var dindex = 0;
var policyId = "";
var policyName = "";
...
function addCourseExperimentTB(guideInfo) {
	...
	//courseExperimentTB.ajax.reload(); // 重新加载
...
function submitCourseCheck(){
	...
	$("#requestType").val("modify");
	if(requestType == "save"){
		successPrompt = "课程保存成功";
		continuePrompt = "确定保存修改吗?"
	}else{
	...
	if(data.length <= 0){
		...
		layer.confirm("您还没添加课程实验,"+continuePrompt, {
			...
			if(coursePicFlag == true){
				$("#startSubmit").trigger("click");
			}else{
				submitCourseInfo(false);
			}
		});
...
function submitCourseInfo(coursePicInfo){
	var coursePicInfoTmp = null;
	if(coursePicFlag == true){
		coursePicInfoTmp = JSON.stringify(coursePicInfo); //转成json串
	}
	
	// 课程图片成功上传后提交课程信息;
	$.ajax({
		url : contextPath +'/course/modifyCourseInfo.do',
		type : 'POST',
		traditional: true,//后台接收数组
		dataType: "json",
		data :{
			'requestType': requestType,
			'coursePicInfo':coursePicInfoTmp,
			'course':formData,
			'groupIdList':groupIdList,
			'experimentList':experimentList,
			'deleteExperimentArray' : deleteExperimentArray
		},
		success : function(data) {
			...
			if(data.resultFlag == true){
				...
				//layer.msg(successPrompt, {icon: 1});
...
function deleteExperiment(obj){
	...
	var experimentId = row.data().experimentId;
	if(experimentId != null){
		deleteExperimentArray[dindex] = experimentId;
		dindex ++;
	}
...
function initCoursePicUpload()
{
	...
	layui.use('upload', function(){
	  ...
	  upload.render({
	    elem: '#coursePic'
	    ,url: contextPath + "/course/modifyCoursePic"
	    ,auto: false
	    //,multiple: true
